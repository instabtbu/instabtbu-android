...
//		}else System.out.println(android.os.Build.VERSION.SDK_INT);
	...
	public void savekebiao(String xueqi,String result)
	{
		try {
			...
			result=result.replaceAll("-1\" ", "-1\" style=\"display: none;\"");
			result=result.replaceAll(zhongjian(result,"<link", ">"),"");
			while(result.indexOf("<script")==-1)result=result.replaceAll(zhongjian(result,"<script", ">"),"");
...
//					  xiangxi.add(myString.substring(0,myString.length()-1));
					  ...
					  if(tempStrings.length==5)
						  {
						  String myString = "";
						  if(tempStrings[3].indexOf("单周")!=-1)myString+="单周\n";
						  else if(tempStrings[3].indexOf("双周")!=-1)myString+="双周\n";
						  myString+=tempStrings[4];
						  didian.add(myString);
						  }
					  else if(tempStrings.length==6)
						  {
						  if(tempStrings[4].indexOf("暂无")==-1)didian.add(tempStrings[4]+"\n"+tempStrings[5]);
						  ...
						  if(tempStrings[8].indexOf("单周")!=-1)myString+="单周\n";
						  else if(tempStrings[8].indexOf("双周")!=-1)myString+="双周\n";
						  else myString+=tempStrings[8]+"\n";
						  ...
						  if(tempStrings[9].indexOf("单周")!=-1)myString+="单周\n";
						  else if(tempStrings[9].indexOf("双周")!=-1)myString+="双周\n";
...
//		{
//			result = POST("http://jwgl.btbu.edu.cn/tkglAction.do?method=goListKbByXs&sql=&xnxqh="+xueqiList.get(i), "");
...
//		}
	...
	public void kebiao(View v) {
	  ...
	  public void handleMessage(Message msg){
	      ...
	      if(msg.what == 1){
	          ...
	          CustomAdapter customAdapter = new CustomAdapter(thisActivity, shijian);  
				...
				.setSingleChoiceItems(xueqiStrings, 0,
				  new DialogInterface.OnClickListener() {
				     public void onClick(DialogInterface a, int which) {  
						...
						editor.commit();
				    	a.dismiss();
				    	dialog = ProgressDialog.show(thisActivity, "登录成功", "正在获取课表……",true,true);
				    	executorService.submit(getkebiaoRunnable);
				     }
				  }
				)
	...
	public void jutikebiao() {
  		new  AlertDialog.Builder(jiaowu_kebiao.this)
	    .setTitle(titlejuti)
		.setMessage(jutiString)
	...
	private class CustomAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {  
            ...
            int myid[] = {R.id.jiaowu_zhouyi,R.id.jiaowu_zhouer,R.id.jiaowu_zhousan,
            		R.id.jiaowu_zhousi,R.id.jiaowu_zhouwu};
            int i;
            try{
            List<TextView> zhouList = new ArrayList<TextView>();
            for(i=0;i<5;i++)
            {
            TextView tempTextView = (TextView) convertView.findViewById(myid[i]);
            zhouList.add(tempTextView);
            if(position%2==0)tempTextView.setBackgroundColor(0xFFF2F2F2);
            tempTextView.setText(mingcheng.get(position*7+i)+"\n"+didian.get(position*7+i));
            int j=0;
            int hang=quhangshu(mingcheng.get(position*7+i)+"\n"+didian.get(position*7+i));
            if(mingcheng.get(position*7+i).length()>5)hang++;
            if(didian.get(position*7+i).length()>12)hang++;
            for(j=0;j<5-hang;j++)
            	tempTextView.append("\n");
            if(xiangxi.get(position*7+i).indexOf("单周")!=-1||xiangxi.get(position*7+i).indexOf("双周")!=-1)
            	{
            ...
            tempTextView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					// TODO Auto-generated method stub
					try{
					jutiString=xiangxi.get(Integer.valueOf(v.getTag().toString()));
					titlejuti=mingcheng.get(Integer.valueOf(v.getTag().toString()));
					if(jutiString.length()!=0)jutikebiao();
					}catch(Exception e)
					{}
				}
			});
    ...
    public String POST(String url,String postdata)
    {
    	String result = "";
	    	...
	    	try{
	    		...
	    		hPost.setEntity(hen);
	    		jiaowuchaxun.myClient.getParams().setParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, 30000);
	    		//请求超时
	    		jiaowuchaxun.myClient.getParams().setParameter(CoreConnectionPNames.SO_TIMEOUT, 30000);
	    		//读取超时
