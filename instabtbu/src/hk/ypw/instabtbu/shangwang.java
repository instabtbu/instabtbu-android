...
//		wifiConfiguration.allowedKeyManagement.set(WifiConfiguration.KeyMgmt.WPA_EAP);
		...
		/**
		 * 在这个函数中找到界面中的元素的实例,没有实例就无法操作界面上的元素
		 * 举个例子,你需要把textview流量上面的字改成12345 MB,你需要这样做:
		 * TextView liuliangTextView = (TextView)findViewById(R.id.shangwang_textliuliang);
		 * liuliangTextView.setText("12345 MB");
		 * 第一行从R文件中寻找到了Id,然后从Id寻找到了View,再将View强制类型转换到TextView
		 * 为什么要强制类型转换呢,因为View没有setText这个方法
		 * 所以我们要这样写
		 */
		...
		if(liuliang.length()!=0)liuliangTextView.setText(liuliang+" MB");
		if(shebei.length()!=0)shebeiTextView.setText(shebei);
		if(find(zidongcha,"z"))zidongchaImageView.setBackgroundResource(R.drawable.shangwang_zidongcha1);
		else zidongchaImageView.setBackgroundResource(R.drawable.shangwang_zidongcha0);
		...
		if(find(zidongcha,"z")&&isWifiConnected(this))executorService.submit(rChaliuliang);
		/**
		 * 如果自动查流量的功能开启了,那我们就把chaliuliang2这个任务投入线程池中
		 * 线程池的语法:
		 * private ExecutorService executorService = Executors.newCachedThreadPool();
		 * executorService.submit(rChaliuliang2);
		 * 第一句定义了线程池,一般放在外面当作全局变量
		 * (如果你想知道它在哪里被定义了,请点击executorService按F3)
		 * 第二句就是把一个Runnable投入到线程池中.
		 * 一个Runnable相当于一个任务(task)
		 * 之后会有Runnable的写法.
		 */
		
		if(find(zidong,"z"))
		{
			zidongdengluImageView.setBackgroundResource(R.drawable.shangwang_zidongdenglu1);
			if(checkser()==false)
				{
				if(isWifiConnected(this))denglu(null);
				}
			/**
			 * 如果自动,那么就给checkbox1(自动登录)设置勾勾
			 * 然后如果服务没有被开启,我们就进行查流量
			 */
		}
		else zidongdengluImageView.setBackgroundResource(R.drawable.shangwang_zidongdenglu0);
		...
		zidongdengluImageView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!find(zidong, "z")){ 
					...
					editor.putString("zidong","f");
					zidongdengluImageView.setBackgroundResource(R.drawable.shangwang_zidongdenglu0);
				}
				editor.commit();
			}
		});
		...
		zidongchaImageView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if(!find(zidongcha, "z")){
					...
					imageView.setBackgroundResource(R.drawable.shangwang_zidongcha1);
				}else{ 
					editor.putString("zidongcha","f");
					imageView.setBackgroundResource(R.drawable.shangwang_zidongcha0);
				}
				editor.commit();
			} 
		});
...
//			ImageView forceImageView = (ImageView)findViewById(R.id.shangwang_imgforce);
...
//		}
	...
	public boolean checkser()
	{
		...
		for (int i=0; i<serviceList.size(); i++) {
			if (find( serviceList.get(i).service.getClassName(),"hk.ypw.instabtbu.ser" ))
			{
				isRunning = true;
				break;
			}
		}
		if(isRunning&&!boolser)stopService();
...
//				}
	...
	public String POST(String url,String postdata)
	{
		...
		{
//			System.out.println("cmd="+zhongjian(postdata, "netlogincmd=","&"));
			try{
...
//					System.out.println("网络延迟:"+shangwang.delaytime);
	...
	public void denglubutton(int a)
	{
		...
		if(a==1)
		{
			...
			zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_zhuangtai0);
		}
		else 
		{
...
//			else if(delaytime<800)zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_yilianjie4);
//			else if(delaytime<1500)zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_yilianjie3);
//			else if(delaytime<3000)zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_yilianjie2);
//			else zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_yilianjie1);
			zhuangtaImageView.setBackgroundResource(R.drawable.shangwang_zhuangtai1);
		}
	}
		...
		public void run(){
			...
			if(connect()){
				...
				if(rec.length==23){
					...
					if(rec!=null){
						SharedPreferences.Editor editor = getSharedPreferences("data", 0).edit();
						editor.putString("num", editText_num.getText().toString());
						editor.putString("psw",editText_psw.getText().toString());
						editor.commit();
...
//		show(gx);
	...
	Runnable rDuankailianjie = new Runnable(){
		...
		public void run(){
			 ...
			 * 数据:textfield=你的学号&textfield2=你的密码&Submit=%CC%E1%BD%BB&nacgicmd=9&radio=1&jsidx=1
			...
			EditText editText_num =(EditText)findViewById(R.id.shangwang_edit_num);
			EditText editText_psw =(EditText)findViewById(R.id.shangwang_edit_psw);
			result = POST("http://self.btbu.edu.cn/cgi-bin/nacgi.cgi",
					"textfield="+editText_num.getText()+"&textfield2="+editText_psw.getText()+"&Submit=%CC%E1%BD%BB&nacgicmd=9&radio=1&jsidx=1");
	...
	Runnable rChaliuliang = new Runnable(){
		...
		public void run(){
			...
			if(rec.length==23){
				...
				char[] msg = user_noip(editText_num.getText().toString(),editText_psw.getText().toString());
				msg=feng(msg, 0x03);
				msg=zhuan(msg);
				send(msg);
				rec = read();
				...
				String yueString = "";
				if(!find(result,"流量"))show(result,1);
				else {
					try{
